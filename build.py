...
def build_base():
    try:
        ...
        # docker build
        subprocess.call('docker build -t myrealtrip:base -f Dockerfile.base .', shell=True)
    finally:
...
def build_local():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t myrealtrip:local -f Dockerfile.local .', shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
...
def build_dev():
    try:
        ...
        # docker build
        subprocess.call('docker build -t myrealtrip:dev -f Dockerfile.dev .', shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
...
def build_production():
    try:
        ...
        subprocess.call('pipenv lock --requirements > requirements.txt', shell=True)
        # docker build
        subprocess.call('docker build -t myrealtrip:production -f Dockerfile.production .',
                        shell=True)
    finally:
        # 끝난 후 requirements.txt파일 삭제
        os.remove('requirements.txt')
